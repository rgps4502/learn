...
agate = Region(780, 1473, 110, 110) --點擊獎盃
...
confim = Region(459, 1265, 150, 90) --選擇到重覆肥料按確認

--打好友
friend_pk_inter = Region(66, 332, 190, 80) --進入好友頁面
pk = Region(781, 643, 95, 1257) --開始PK
victory = Region(314, 844, 418, 244) --戰鬥結束勝利
lose = Region(484, 1000, 110, 66) --戰鬥結束失敗

--金幣買箱子
money_box = Region(938, 503, 100, 90) --買體力 點主畫面的箱子
touch_money = Region(280, 1831, 50, 100)
money_buy = Region(629, 1728, 80, 90)
pvp_start1 = Region(714, 1253, 60, 60)
--==========  main program ===========
-- function pvp() --打pvp
    ...
    while (true) do
        if pk:exists('pk.png', 0) then
            ...
            until victory:existsClick('victory.png', 0) or lose:existsClick('lose.png', 0) or
                friend_pk_inter:exists('pk_inter.png', 0)
            sleep(5)
        else
            repeat
...
function food() --練肥
    ...
    while (true) do
        ...
        until food_full:exists('food_full.png', 0) or MXA:exists('food_MAX.png', 0) or
            fight_config:exists('fight_config.png', 0)
        if food_full:exists('food_full.png', 0) then --如果腳色滿了 執行換瑪瑙
            ...
            sleep(3)
            agate_exit:existsClick('agate_exit.png', 20)
...
function food20_max20() --當肥料20等換肥
    ...
    while not star2:exists('star2.png', 0) do
        if not star2:exists('star2.png', 0) then
            ...
            confim:existsClick('confim.png', 0)
        end
    end
...
function buy_h() ---用錢買體力
    ...
    click(Location(706, 1288))
    if money_buy:exists('money_buy.png', 0) then
        while true do
            money_buy:existsClick('money_buy.png', 0)
            if pvp_start1:exists('choose_ok.png', 0) then
                pvp_start1:existsClick('choose_ok.png', 10)
                pvp_start1:waitVanish('choose_ok.png', 60)
            end
        end
    end
end
